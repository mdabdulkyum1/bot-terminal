...
export class AutoComplete {
  constructor() {
    ...
    this.projectCommands = [
      'analyze', 'summary', 'read', 'edit', 'project'
    ];
  ...
  complete(line) {
    ...
    // Handle AI commands
    if (command.startsWith('ai') || command.startsWith('?') || command === 'ask') {
      return this.completeAICommand(line);
    }

    // Handle project commands
    if (this.projectCommands.includes(command)) {
      return this.completeProjectCommand(command, lastToken);
    }
  ...
  completeCommand(command) {
    const matches = [...this.commonCommands, ...this.aiCommands, ...this.projectCommands]
  ...
  completeProjectCommand(command, lastToken) {
    if (command === 'read' || command === 'edit') {
      ...
      const suggestions = [
        ...
        'project How can I improve this?'
      ];
      const matches = suggestions.filter(suggestion => 
        suggestion.toLowerCase().startsWith(`project ${lastToken}`.toLowerCase())
      );
      return [matches, line];
    }
    return [[], line];
  ...
  getSmartSuggestions(line, history = []) {
    ...
    if (history.length > 0) {
      ...
      if (lastCommand.includes('analyze')) {
        suggestions.push('summary', 'read ', 'edit ');
      }
    }
